...
description: "Write, run, and analyze structured test suites for Agentforce agents. TRIGGER when: user writes or modifies test spec YAML or test metadata (AiTestingDefinition for NGT / Agentforce Studio, or legacy AiEvaluationDefinition); runs sf agent test create, run, run-eval, or results commands; retrieves, hand-edits, or deploys AiTestingDefinition metadata XML; asks about test coverage strategy, metric/scorer selection, or custom evaluations; interprets test results or diagnoses test failures; asks about batch testing, regression suites, or CI/CD test integration. DO NOT TRIGGER when: user creates, modifies, previews, or debugs .agent files (use developing-agentforce); deploys or publishes agents; writes Agent Script code; uses sf agent preview for development iteration; analyzes production session traces (use observing-agentforce); requests OWASP, security, or red-team testing (use securing-agentforce)."
...
### NGT (Agentforce Studio) — `AiTestingDefinition` Metadata

> Full reference: `references/ngt-metadata-reference.md`

The YAML spec above targets the **legacy Testing Center** (`AiEvaluationDefinition`). NGT
(Agentforce Studio) is the newer path: its metadata type is `AiTestingDefinition` and its runner
is `agentforce-studio`. Detect NGT support with
`sf agent test create --help | grep -i test-runner` (flag present = NGT-capable CLI) and a project
`sourceApiVersion >= 66.0`.

Work at the `AiTestingDefinition` **XML layer** when the suite is already metadata — retrieved from
an org, committed to a repo, or generated by other tooling — or when you need a field the spec
compiler doesn't emit. The headline case is **custom scorers**: a customer-defined evaluator is
attached only in XML, as
`<scorer><name>CustomScorer2</name><scorerType>Custom</scorerType></scorer>` (no `expectedValue`) —
the spec compiler has no representation for it. The `<name>` is the DeveloperName of a separate
`AiAgentScorerDefinition` metadata file (which defines the scorer's engine and pass/fail outcomes);
the test suite only references it by name. The org-gated `hallucination_score` scorer is similarly
XML-only.

This path is still 100% CLI-driven — `sf project retrieve start` / `sf project deploy start` to
move the XML, `sf agent test run` / `results` to execute. The NGT Connect API
(`POST /einstein/ai-testing/runs`, `GET .../runs/:runId`, `GET .../runs/:runId/results`) is what
those commands call internally; **never issue raw `curl` against those endpoints from this skill.**

```bash
# Retrieve an existing suite as XML, edit in source control, redeploy
sf project retrieve start --json --metadata "AiTestingDefinition:<Name>" -o <org>
#   ...hand-edit force-app/main/default/aiTestingDefinitions/<Name>.aiTestingDefinition-meta.xml...
sf project deploy start --json --metadata "AiTestingDefinition:<Name>" -o <org>

# Then run via the same CLI
sf agent test run --json --api-name <Name> --wait 10 --result-format json -o <org>
```

`references/ngt-metadata-reference.md` documents the full XML schema (root `<AiTestingDefinition>`,
`<testCase>`/`<number>`/`<inputs>`, the `<contextVariable>` and `<conversationHistory>` input
sub-types, `<scorer>`/`<scorerType>`/`<expectedValue>`), the OOTB scorer catalog, custom scorers
(both metadata types), and a pre-deploy checklist.
